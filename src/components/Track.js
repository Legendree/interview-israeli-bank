...
export default function Track({
  ...
  return (
    <div className='track_container'>
      <div className={`track_img_container ${isFetching && 'shimmer'}`}>
        {!isFetching && (
        ...
        {trackDuration && (
          <span className='track_duration_label'>
            {convertToTime(trackDuration)}
          </span>
        )}
        <span className='track_number_label'>{trackNumber}</span>
      </div>
      <span className='track_label'>{trackName}</span>
    </div>
  );
}
